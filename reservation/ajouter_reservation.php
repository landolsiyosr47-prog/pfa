...
<head>
    ...
    <title>RéEmploi BTP — Ajouter une Réservation</title>
...
<body>
    ...
    <nav class="navbar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="index.php" class="btn-list">Liste Réservations</a></li>
        </ul>
    </nav>
    ...
    <main class="main-content">
        <div class="form-card">
            ...
            <form action="insert_reservation.php" method="POST">
                ...
                <span class="label-title">Choisir le matériau</span>
                <select name="id_materiau" required>
                    <option value="" disabled selected>Sélectionnez un matériau...</option>
                    <?php
                    $sql = "SELECT id_materiau, nom_materiau FROM materiau";
                    $result = $conn->query($sql);
                    while($row = $result->fetch_assoc()){
                        echo "<option value='".$row['id_materiau']."'>".$row['nom_materiau']."</option>";
                    }
                    ?>
                </select>

                <span class="label-title">Utilisateur (Client)</span>
                <select name="id_user" required>
                    <option value="" disabled selected>Sélectionnez l'utilisateur...</option>
                    <?php
                    $sql = "SELECT id_user, nom_user, prenom_user FROM utilisateur";
                    $result = $conn->query($sql);
                    while($row = $result->fetch_assoc()){
                ...
                <span class="label-title">Statut actuel</span>
    ...
    <footer class="footer">
        <div class="footer-inner">
            ...
            <p style="font-size: 0.9rem; opacity: 0.8;">Panel de gestion des réservations matériaux.</p>
        </div>
